...
Create a new empty workspace. No prompts — instant.

**Behavior:**
1. Generate a random ID (adjective-noun, e.g. `calm-delta`)
2. If a name argument is provided, associate it with the workspace
3. Create workspace directory at `~/.flow/workspaces/<id>/`
4. Write empty `state.yaml` (no repos)
5. Print success message with next step
...
✓ Created workspace calm-delta (my-project)
...
flow init
# → Creates ~/.flow/workspaces/calm-delta/ with empty state
