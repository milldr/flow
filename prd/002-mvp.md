...
All commands are global — run from anywhere.

| Command | Description |
|---------|-------------|
| `flow init` | Create new workspace (interactive) |
...
**Flow:**
1. Prompt: "What are you working on?" → `description`
2. Auto-generate workspace name from description (slugified)
3. Prompt: "Workspace name:" (editable, shows auto-generated default)
4. Prompt: "Add a repo?" → repo URL
5. Prompt: "Branch for <repo>?" → branch name
6. Repeat repo prompts until user enters blank
7. Create workspace directory at `~/.flow/workspaces/<name>/`
8. Write `state.yaml`
9. Print success message with next steps

**Output:**
```
✅ Created workspace: vpc-ipv6
   Location: ~/.flow/workspaces/vpc-ipv6/

Next steps:
  flow state vpc-ipv6     # Edit state file
...
**Output:**
```
📦 Rendering workspace: vpc-ipv6

  [1/2] github.com/cloudposse/atmos
        └── Worktree: ./atmos (feat/ipv6-support) ✓

  [2/2] github.com/cloudposse/docs
        └── Worktree: ./docs (feat/ipv6-support) ✓
...
└── workspaces/
    └── <workspace-name>/
        ├── state.yaml           # Workspace state
        ├── <repo-a>/            # Worktree (created by render)
        └── <repo-b>/            # Worktree
```
...
flow init
# → Prompts for description, name, repos
# → Creates ~/.flow/workspaces/vpc-ipv6/state.yaml
...
| Feature | Planned For |
|---------|-------------|
| Shared skills/symlinks | v0.2 |
| Workspace templates | v0.3 |
| TUI dashboard | v0.4 |
...
- [ ] `flow init` creates workspace directory and valid state.yaml
